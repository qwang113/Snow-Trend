...
# NO TEMPERATURE VERSION (lat + elev only)
# ================================================================
...
def build_dataset(event):
    ...
    for i in range(N):
        ...
        rows_e += [i,i]
        cols_e += [K_total*S, K_total*S+1]
        vals_e += [t_lat[i], t_elev[i]]
...
def run_chain(chain_id, data, event):
    ...
    for it in tqdm(range(total_iters),
        ...
        gamma = curr_eta[K_total*S:K_total*S+2]
        X_factor = X_eta_base[:, K_total*S:K_total*S+2]
    ...
    with open(BASE_DIR / f"{event}_weekly_cov_no_temp_chain{chain_id}.pkl","wb") as f:
