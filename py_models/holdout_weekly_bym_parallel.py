...
def run_chain(chain_id, data, event):
    ...
    with open(OUTPUT_DIR / f"{event}_weekly_train38_chain{chain_id}.pkl","wb") as f:
...
def predict_chain(chain_id):
    ...
    with open(OUTPUT_DIR / f"p01_weekly_train38_chain{chain_id}.pkl", "rb") as f:
        d01 = pickle.load(f)
    with open(OUTPUT_DIR / f"p10_weekly_train38_chain{chain_id}.pkl", "rb") as f:
        d10 = pickle.load(f)

    eta01 = d01["eta"][:, ::PRED_THIN].reshape(8, S, 334)
    tau01 = d01["tau"][:, ::PRED_THIN].reshape(8, period, 334)
    eta10 = d10["eta"][:, ::PRED_THIN].reshape(8, S, 334)
    ...
    output = OUTPUT_DIR / f"pred_weekly_bym_train38_test14_chain{chain_id}.npy"
...
def main():
    ...
    Parallel(n_jobs=n_chains, backend="loky", batch_size=1)(
        delayed(predict_chain)(i) for i in range(n_chains)
    )
