...
alpha <-  theta_a1 + theta_a2
alpha_s <-  theta_a1s + theta_a2s
...
for (time in 1:(TT-1)) {
  P[, time, 1, 2] <- inv_logit(beta_0 + beta_1 * cos(2*pi*time/period) + beta_2 * sin(2*pi*time/period) + alpha * time)
  P[, time, 1, 1] <- 1 - P[, time, 1, 2]
  P[, time, 2, 1] <- inv_logit(beta_0s + beta_1s * cos(2*pi*time/period) + beta_2s * sin(2*pi*time/period)+ alpha_s * time)
...
saveRDS(alpha, file = "alpha.Rda")
saveRDS(alpha_s, file = "alpha_s.Rda")

# # Test whether theta is spatially correlated
# test_thetas <- data.frame("long" = coords[,1], "lat" = coords[,2], "value" = as.vector(alpha))
